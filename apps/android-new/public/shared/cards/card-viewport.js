 ...
 * 
 * AHORA:
 *   - Cada instancia tiene un viewId único (data-view-id)
 *   - data-channel presente en host + shadow
 *   - Evento ver-sponsor incluye channel en detail
 *   - Temas de badge diferenciados por canal
 * 
 * Uso:
 *   <card-viewport
 *     channel="vas_ir"
...
class CardViewport extends HTMLElement {
  ...
  constructor() {
    ...
    this._post = null;
    this._stage = 's1';
    this._viewId = `cvp-${++CardViewport.#counter}-${Date.now()}`;
  }
  ...
  attributeChangedCallback(name, oldVal, newVal) {
    if (name === 'channel') {
      ...
      this.setAttribute('data-channel', newVal);
      this.setAttribute('data-view-id', this._viewId);
    }
    if (name === 'stage') {
    ...
    if (name === 'post') {
      try {
        ...
        if (this._post && this._post.id) {
          this.setAttribute('data-card-ref', this._post.id);
        }
      } catch (e) {
        this._post = null;
      }
  ...
  async #render() {
    ...
    // 4. Renderizar en shadow DOM con view ID y channel como data attrs
    shadow.innerHTML = `
      ...
      <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        /* Badge color por canal */
        .badge {
          position: absolute; top: 12px; left: 12px; z-index: 15;
          padding: 4px 12px; border-radius: 999px;
          font-size: 11px; font-weight: 700;
          color: var(--badge-color,#fff);
          background: ${channel === 'arre' ? 'linear-gradient(135deg, #9333ea, #6366f1)' : 'var(--badge-bg,#f97316)'};
          pointer-events: none;
        }
        .main-container {
        ...
        .media-box.is-s2 { bottom: 250px !important; }
        .card-actions.is-s2 { display: none !important; }
        .glass-back.is-s2, .glass-front.is-s2 { display: none !important; }
        .s2-back {
      ...
      <div class="canvas t0" id="canvasContainer" data-view-id="${this._viewId}" data-channel="${channel}">
    ...
    // Media-box ahora está fuera de ambos stages, compartida.
    // S1: visible normalmente. S2: oculta S1, muestra S2 (overlay comentarios).
    // Media-box siempre visible, solo cambia su bottom constraint.
    const stageS1 = shadow.querySelector('#stageS1');
    const stageS2 = shadow.querySelector('#stageS2');
    if (stageS1) stageS1.style.display = isS2 ? 'none' : 'block';
    if (stageS2) stageS2.style.display = isS2 ? 'block' : 'none';

    // Aplicar clase is-s2 a elementos compartidos para posicionamiento CSS
    const mediaBox = shadow.querySelector('.media-box');
    const glassBack = shadow.querySelector('.glass-back');
    const glassFront = shadow.querySelector('.glass-front');
    const cardActionsEl = shadow.querySelector('.card-actions');
    [mediaBox, glassBack, glassFront, cardActionsEl].forEach(el => {
      if (el) el.classList.toggle('is-s2', isS2);
    });

    // 6. Inicializar interactividad
    this.#initInteraction(post, isS2, channel);
    } catch (e) {
  ...
  #initInteraction(post, isS2, channel) {
    const shadow = this.shadowRoot;
    if (!shadow) return;

    const media = post.media || [];
    const images = media.length > 0 ? media : ['/shared/cards/placeholder.jpg'];
    const ch = channel || this._channel || 'vas_ir';
    ...
    function si(i) {
      ci = i;
      const url = images[i];
      img.src = url && typeof url === 'string' ? url : '/shared/cards/placeholder.jpg';
    ...
    if (!isS2) {
      ...
      // Botón Ver Sponsor (con channel en detail)
    ...
    if (isS2) {
      ...
      // Input de comentario (con channel en detail)
      if (commentInput) {
          ...
          if (e.key === 'Enter') {
            ...
            if (text) {
              this.dispatchEvent(new CustomEvent('send-comment', {
                detail: { postId: post.id, text, channel: ch },
                bubbles: true, composed: true,
              }));
              commentInput.value = '';
            }
