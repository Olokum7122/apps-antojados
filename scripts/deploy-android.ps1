#!/usr/bin/env pwsh
# ═══════════════════════════════════════════════════════════════
# deploy-android.ps1 — Script OFICIAL de build+deploy Android
#
# SOLO este script puede ejecutar builds de Android.
# Cualquier intento directo de 'quasar build' será BLOQUEADO
# por el guardian wrapper.
# ═══════════════════════════════════════════════════════════════
param(
    [switch]$SkipAudit,
    [switch]$Dev
...
Write-Host " GUARDIAN — Deploy Android (oficial)" -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host ""
...
$sessionToken = "android-$((Get-Date -Format 'yyyyMMddHHmmss'))-$([System.IO.Path]::GetRandomFileName().Replace('.',''))"
...
# ── Verificar .env ──────────────────────────────────────────────
$envFile = "$PSScriptRoot\..\apps\android-new\.env.production"
if (Test-Path $envFile) {
    ...
    if ($apiUrl -ne 'https://api.antojadosmx.mx') {
        Write-Error " ❌ .env.production tiene VITE_API_URL='$apiUrl'. Debe ser 'https://api.antojadosmx.mx'"
        exit 1
    }
    Write-Host " ✅ .env.production verificado." -ForegroundColor Green
} else {
    Write-Warning " ⚠  No se encontró .env.production para Android."
}

# ── Build Android ──────────────────────────────────────────────
$projectRoot = "$PSScriptRoot\..\apps\android-new"
Write-Host ""
Write-Host " 🏗  Construyendo Android App..." -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Error " ❌ Build de Android falló."
        exit 1
    }

    Write-Host ""
    Write-Host " ✅ Build de Android completado exitosamente." -ForegroundColor Green
    Write-Host " 📱 APK/AAB listo en: apps/android-new/dist/capacitor/android" -ForegroundColor Green
} finally {
    Pop-Location
...
Write-Host ""
Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Cyan
Write-Host " Deploy Android completado." -ForegroundColor Cyan
Write-Host "═══════════════════════════════════════════════════════════════" -ForegroundColor Cyan