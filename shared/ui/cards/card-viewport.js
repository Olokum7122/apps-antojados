...
class CardViewport extends HTMLElement {
  ...
  async #render() {
    ...
    try {
      ...
      // 1. Obtener template cacheado
      let entry = CardViewport.#cache.get(channel);
      if (!entry) {
        try {
          const htmlUrl = channel === 'arre' ? '/shared/cards/indexarre.html' : '/shared/cards/indexvasir.html';
          console.log('[DIAG card-viewport] fetching template:', htmlUrl);
          const resp = await fetch(htmlUrl);
          entry = { html: await resp.text(), loaded: true };
          CardViewport.#cache.set(channel, entry);
          console.log('[DIAG card-viewport] template fetched, length:', entry.html.length);
        } catch (err) {
          console.error('[DIAG card-viewport] FETCH ERROR:', err.message);
      ...
      <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .s2-rail {
          ...
          gap: 2px; background: rgba(10,12,20,0.55);
        ...
        .rail-count { font-size: 8px; font-weight: 700; line-height: 1; color: rgba(255,255,255,0.5); }
        .rail-divider { width: 24px; height: 1px; background: rgba(255,255,255,0.1); margin: 2px 0; }
        /* ── FAB grande dentro del rail ── */
        .rail-btn--fab {
          width: 50px; height: 50px;
          background: var(--btn-bg,#f59e0b); color: var(--btn-color,#0a0c12);
          border-radius: 14px; margin-top: 2px;
        }
        .rail-btn--fab:hover { opacity: 0.85; background: var(--btn-bg,#f59e0b); color: var(--btn-color,#0a0c12); }
        .rail-btn--fab:active { transform: scale(0.90); }
        /* ── Stage visibility ── */
  ...
  #initInteraction(post, isS2) {
    ...
    // ─── Solo S2: botón retorno + comentarios + rail ───
    if (isS2) {
      ...
      // FAB Publicar
      const btnFab = shadow.querySelector('#btnFabPublicar');
      if (btnFab) {
        btnFab.addEventListener('click', () => {
          this.dispatchEvent(new CustomEvent('open-publish', {
            detail: { sponsorId: post.sponsorId, channel: this._channel },
            bubbles: true, composed: true,
          }));
        });
      }
      ...
      // Botones del rail
      const railBtns = shadow.querySelectorAll('.rail-btn[data-action]');
