<template>
  <section class="que-pex-post-fullscreen-view">
    <feed-fullscreen-base
      ...
      media-fit="contain"
      :show-comments="false"
      subdim-ik="QUE_PEX_FULLSCREEN"
      subdim-pc="ANTOJADOS.PARA_TI.QUE_PEX"
      subdim-type="FULLSCREEN"
      subdim-applies-to="all"
      code-component="QUE_PEX.FULLSCREEN"
      @close="goBack"
      @rail-action="onRailAction"
    >
      <template #overlay="{ post: item }">
        <div class="que-pex-post-fullscreen-view__copy">
          <button type="button" class="que-pex-post-fullscreen-view__author" @click.stop="goUser(item)">
            @{{ item.authorHandle || item.author }}
          </button>
          <button
            v-if="item.venueName || item.venue"
            type="button"
            class="que-pex-post-fullscreen-view__venue"
            @click.stop="goVenue(item)"
          >
            <q-icon name="place" size="13px" />
            {{ item.venueName || item.venue }}
          </button>
          <q-badge color="primary" text-color="dark">{{ item.momentTag || 'QUE PEX' }}</q-badge>
          <p>{{ item.caption }}</p>
        </div>
      </template>
      ...
      <template #quickInput>
        <form
          class="que-pex-post-fullscreen-view__review"
          subdim-ik="QUE_PEX_REVIEW_PANEL"
          subdim-pc="ANTOJADOS.PARA_TI.QUE_PEX"
          subdim-type="SUB_COMPONENT"
          subdim-applies-to="all"
          data-code-component="QUE_PEX.REVIEW_PANEL"
          @submit.prevent="submitReview"
        >
          <input
            v-model.trim="reviewDraft"
            class="que-pex-post-fullscreen-view__review-input"
...
watch(
  ...
  async ([postId, userId]) => {
    ...
    const directPost = await socialFeedService.getById(resolvedPostId, 'que-pex')
    ...
    if (resolvedUserId) {
      const userPosts = await socialFeedService.listByUser(resolvedUserId, 'que-pex')
...
function goBack() {
  ...
  if (resolvedUserId) {
    router.push(`/red/pa-ti/que-pex/usuario/${resolvedUserId}?post_id=${post.value?.id || ''}`)
    return
  }
...
function goUser(item) {
  ...
  router.push(`/red/pa-ti/que-pex/usuario/${item.userId}?post_id=${item.id}`)
}
...
function syncAction(eventType, payload) {
  return pushEvent({
    ...
    publisherUserId: post.value?.publisherUserId,
...
function submitReview() {
  if (reviewDraft.value) {
    void syncAction('post_comment', { text: reviewDraft.value, source: 'que_pex_review' })
  }
  reviewDraft.value = ''
}
</script>
...
<style scoped>
.que-pex-post-fullscreen-view {
  min-height: 100%;
  background: #000;
}

.que-pex-post-fullscreen-view__copy {
  display: grid;
  justify-items: start;
  gap: 6px;
}

.que-pex-post-fullscreen-view__copy p {
  margin: 0;
  color: rgba(255, 255, 255, 0.86);
}

.que-pex-post-fullscreen-view__author,
.que-pex-post-fullscreen-view__venue {
  border: 0;
  padding: 0;
  color: #fff;
...
.que-pex-post-fullscreen-view__review {
  display: grid;
...
.que-pex-post-fullscreen-view__review-input {
  width: 100%;
  min-height: 38px;
  padding: 0 12px;
  border: 0;
