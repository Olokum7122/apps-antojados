      ...
      >
        <slot name="item" :item="item" :index="index">
          <slot v-if="item?._sponsor" name="sponsor" :item="item" :index="index">
            <div class="base-feed-gallery__sponsor">Sponsor</div>
          </slot>
...
const classes = resolveBaseComponentClasses('feedGallery', {
...
const rootClass = computed(() => [
  classes.root,
  `base-feed-gallery--stage-${props.stage.toLowerCase()}`,
  `base-feed-gallery--${props.variant}`,
  effectiveModel.value === 'androidMosaic'
    ? 'base-feed-gallery--android'
    : 'base-feed-gallery--uniform',
])
