<template>
  <section class="sponsor-vasir-page">
    ...
    <div v-if="isS2" class="sponsor-vasir-page__sponsor-header">
    ...
    <div v-if="cardPosts.length" class="sponsor-vasir-page__feed">
      <card-viewport
    ...
    <div v-if="loading" class="sponsor-vasir-page__loading">
      <div v-for="i in 3" :key="'skel-' + i" class="sponsor-vasir-page__skeleton" />
    </div>
...
Tu publicacion llega a antojados de toda la ciudad."
    ...
    <post-action-rail-base
      ...
      :show-counts="false"
      :actions="railActions"
      class="sponsor-vasir-page__rail"
      @action="onRailAction"
    />
  </section>
</template>
...
const {
  ...
  zoneScopeCode, requestDeviceGeo,
...
function loadFeed() {
  ...
  if (!route.path.includes('/antojo/vas-ir/')) {
    loading.value = false
    return
  }

  const sid = publisherId.value
  const feedScope = CHANNEL as 'vas_ir'
  const loadPromise = sid
    ? bizFeedService.listByPublisher(sid, feedScope, { limit: 30 })
    : bizFeedService.list({
        feedScope,
        limit: 30,
  ...
  loadPromise
    .then((result) => {
      if (route.path.includes('/antojo/vas-ir/')) posts.value = result
    })
    .catch((err) => {
      if (route.path.includes('/antojo/vas-ir/')) {
    ...
    .finally(() => {
      if (route.path.includes('/antojo/vas-ir/')) loading.value = false
    })
}

function onVerSponsor(event: CustomEvent) {
  const sponsorId = event.detail?.sponsorId
  if (sponsorId) router.push(`/antojo/vas-ir/negocio/${sponsorId}`)
}

function onOpenS3(postId: string) {
  const sid = publisherId.value
  router.push({ path: `/antojo/vas-ir/negocio/${sid}/post/${postId}` })
}

function goBackToS1() { router.push('/antojo/vas-ir/gallery') }
function goToPublish() { router.push('/antojo/publicar') }
...
onMounted(() => {
  // Forzar detección GPS al montar para obtener scope ciudad real
...
<style scoped>
.sponsor-vasir-page { width: 100%; min-height: 100vh; display: flex; flex-direction: column; align-items: center; }
.sponsor-vasir-page__type-filter { display: flex; gap: 6px; padding: 8px 10px; overflow-x: auto; background: #0d0f16; width: 100%; }
.sponsor-vasir-page__sponsor-header { display: flex; align-items: center; gap: 12px; padding: 12px 16px; width: 100%; background: #0d0f16; }
.sponsor-vasir-page__back-btn { width: 52px; height: 52px; min-width: 52px; min-height: 52px; border-radius: 50%; border: none; background: rgba(0,0,0,0.42); backdrop-filter: blur(8px); box-shadow: 0 8px 20px rgba(0,0,0,0.34); color: #fff; cursor: pointer; display: flex; align-items: center; justify-content: center; touch-action: manipulation; }
.sponsor-vasir-page__sponsor-name { font-size: 18px; font-weight: 700; margin: 0; }
.sponsor-vasir-page__feed { display: flex; flex-direction: column; align-items: center; gap: 16px; padding: 16px 0; width: 100%; max-width: 400px; }
.sponsor-vasir-page__post { width: 100%; }
.sponsor-vasir-page__loading { display: flex; flex-direction: column; align-items: center; gap: 16px; padding: 16px 0; width: 100%; max-width: 400px; }
.sponsor-vasir-page__skeleton { width: 100%; height: 350px; border-radius: 28px; background: linear-gradient(90deg,#1a1d2e 0%,#2a2d3e 50%,#1a1d2e 100%); background-size: 200% 100%; animation: skeletonPulse 1000ms ease-in-out infinite; }
@keyframes skeletonPulse { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }
.sponsor-vasir-page__rail {
  position: fixed;
  bottom: 100px;
  left: 50%;
